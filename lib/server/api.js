...
Cluster.connect = function(connUrl, options) {
  ...
  if(matched) {
    ...
    var discoveryBackend = this._discoveryBackends[backendName];
    if(!discoveryBackend) {
    ...
    Discovery.connect(connUrl, discoveryBackend, this, options);
...
Cluster.discoverConnection = function discoverConnection(name, ddpOptions) {
  ...
  var watcher = new ConnectionWatcher(Discovery, name, options);
