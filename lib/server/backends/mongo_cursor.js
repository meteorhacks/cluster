...
MongoBackend.Cursor.prototype.fetch = function() {
  ...
  if(autoupdateVersion) {
    ...
    if(result.length < this._limit) {
      ...
      var services = this.mergeAutoUpdate(result, oldResult, this._limit);
    } else {
      var services = result;
    }
  } else {
  ...
  var healthyServices = services.filter(this._checkHealthy);
  healthyServices.sort(function(a, b) {
...
MongoBackend.Cursor.prototype.mergeAutoUpdate =
