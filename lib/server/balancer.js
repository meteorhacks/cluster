...
function _sendSockJsInfo(req, res, endpoint, balancer) {
  var endpointHash = ClusterManager.discovery.endpointToHash(endpoint);
  var base_url = urlResolve(balancer, "cluster-ddp/" + endpointHash);
...
Balancer._rewriteDdpUrl = function _rewriteDdpUrl(req) {
  var regExp = /cluster-ddp\/(\w*)\//;
  var parsedUrl = req.url.match(regExp);
  if(parsedUrl) {
    var endpointHash = parsedUrl[1];
    req.url = req.url.replace(regExp, "sockjs/");
    return endpointHash;
  }
};
...
Balancer.handleHttp = function handleHttp(req, res) {
  ...
  var endpointHash = Balancer._rewriteDdpUrl(req);
  if(!endpointHash) {
    // seems like this is just static resources
    // we can get the endpointHash from the cookie
    endpointHash = cookies.get('cluster-endpoint');
  }

  var endpoint = Balancer._pickEndpoint(endpointHash, cookies);
  if(!endpoint) {
    console.log("no endpoint:", req.url);
    return false;
  }

  var balancer = ClusterManager.discovery.pickBalancer();
  // if there is a balancer, we need to send sockjs traffic over
  // the balancer
  if(balancer && /info/.test(req.url)) {
